...
namespace SimpleInventorySort
{
    public class Settings
    {
        ...
        private bool m_gatherEntities = true;
        private bool m_getInventories = true;
        private bool m_chatToggle = true;
        ...
        public bool GatherEntities
        {
            get { return m_gatherEntities; }
            set
            {
                m_gatherEntities = value;
                Save();
            }
        }
        ...
        {
            get { return m_getInventories; }
            set
            {
                m_getInventories = value;
                Save();
            }
        }
            ...
            try
            {
                if (MyAPIGateway.Utilities.FileExistsInLocalStorage("Settings.txt", typeof(Settings)))
                {
                    using (TextReader reader = MyAPIGateway.Utilities.ReadFileInLocalStorage("Settings.txt", typeof(Settings)))
                    {
                        ...
                        m_addToQueue = true;
                        bool.TryParse(reader.ReadLine(), out m_addToQueue);

                        m_gatherEntities = true;
                        bool.TryParse(reader.ReadLine(), out m_gatherEntities);

                        m_getInventories = true;
                        bool.TryParse(reader.ReadLine(), out m_getInventories);

                        m_chatToggle = true;
                        bool.TryParse(reader.ReadLine(), out m_chatToggle);
                    }
                }
            }
            catch (Exception ex)
        ...
        public void Save()
        {
            ...
            try
            {
                using (TextWriter writer = MyAPIGateway.Utilities.WriteFileInLocalStorage("Settings.txt", typeof(Settings)))
                {
                    ...
                    writer.WriteLine(m_addToQueue);
                    writer.WriteLine(m_gatherEntities);
                    writer.WriteLine(m_getInventories);
                    writer.WriteLine(m_chatToggle);
                }
            }
            catch (Exception ex)
            {
