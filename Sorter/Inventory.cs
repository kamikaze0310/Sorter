...
namespace SimpleInventorySort
{
    ...
    public struct Inventories
    {
        ...
        public List<IMyTerminalBlock> pulleeBlocks;
    }
    ...
    {
        private static ConcurrentDictionary<long, List<SortDefinitionItem>> m_cargoDictionary = new ConcurrentDictionary<long, List<SortDefinitionItem>>();
        private static ConcurrentDictionary<MyDefinitionBase, List<IMyEntity>> m_splitGroups = new ConcurrentDictionary<MyDefinitionBase, List<IMyEntity>>();
        private static ConcurrentDictionary<MyDefinitionBase, List<IMyEntity>> m_shareGroups = new ConcurrentDictionary<MyDefinitionBase, List<IMyEntity>>();
        ...
        private static volatile bool m_rebuild = true;
        ...
        private static List<IMyEntity> onlineEntities = new List<IMyEntity>();
        private static ConcurrentQueue<IMyEntity> smallProcesses = new ConcurrentQueue<IMyEntity>();
        private static ConcurrentQueue<Inventories> inventories = new ConcurrentQueue<Inventories>();
        private static List<IMyEntity> largeProcesses = new List<IMyEntity>();
        private static volatile bool m_getOnlineEntities = true;
        private static volatile bool queueInventories = false;
        ...
        public static int QueueEntities
        {
            get { return smallProcesses.Count; }
        }

        public static bool QueueInventories
        {
            get { return queueInventories; }
            set { queueInventories = value; }
        }
        ...
        public static void ChatSortInventory(IMyCubeGrid grid)
        {
            MyAPIGateway.Parallel.StartBackground(() =>
            {
                try
                {
                    if (!Settings.Instance.ChatToggle) return;
                    List<IMyEntity> myEntities = new List<IMyEntity>();
                    IMyEntity entity = grid as IMyEntity;
                    if (entity == null) return;

                    myEntities.Add(entity);
                    RebuildSortListFromEntities(myEntities);
                    ResetSplitGroups();
                    smallProcesses.Enqueue(entity);
        ...
        public static void NewSortInventory()
        {
            MyAPIGateway.Parallel.StartBackground(() =>
            {
                try
                {
                    if (!Settings.Instance.GatherEntities) return;
                    if (m_queueReady) return;
                    if (smallProcesses.Count == 0) m_getOnlineEntities = true;
                    else return;
                    ...
                    foreach (var player in playerList)
                    {
                        ...
                        playerIdList.Add(player.IdentityId);
                    }
                    ...
                    /*if (smallProcesses.Count != 0 && index >= smallProcesses.Count - 1)
                    {
                        index = -1;
                        smallProcesses.Clear();
                        m_getOnlineEntities = true;
                        onlineEntities.Clear();
                    }*/

                    List<long> alreadyProcessed = new List<long>();

                    if (m_getOnlineEntities)
                    {
                        m_getOnlineEntities = false;
                        //MyVisualScriptLogicProvider.ShowNotificationToAll($"Gathering Entities", 5000, "Green");
                        List<IMyPlayer> playerList = new List<IMyPlayer>();
                        List<long> playerIdList = new List<long>();
                        MyAPIGateway.Players.GetPlayers(playerList);
                        ...
                        foreach (IMyEntity entity in entities)
                        {
                            ...
                            try
                            {
                                ...
                                if (bigowners.Count == 0) continue;

                                bool processEntity = false;
                                IMyFaction blockFaction = null;
                                foreach (var owners in bigowners)
                                {
                                    if (owners == 0) continue;

                                    if (playerIdList.Contains(owners))
                                    {
                                        processEntity = true;
                                        break;
                                    }

                                    blockFaction = MyAPIGateway.Session.Factions.TryGetPlayerFaction(owners);
                                    if (blockFaction != null) break;
                                }
                                ...
                                if (blockFaction != null)
                                {
                                    foreach (var id in playerIdList)
                                    {
                                        ...
                                        if (playerFaction == blockFaction && playerFaction != null && blockFaction != null)
                                        {
                                            processEntity = true;
                                            break;
                                        }
                                ...
                                if (!processEntity) continue;
                                onlineEntities.Add(entity);
                                /*if (alreadyProcessed.Contains(cubeGrid.EntityId)) continue;
                                var gridGroup = MyAPIGateway.GridGroups.GetGroup(cubeGrid, GridLinkTypeEnum.Physical);
                                foreach (var grid in gridGroup)
                                {
                                    if (alreadyProcessed.Contains(grid.EntityId)) continue;
                                    alreadyProcessed.Add(grid.EntityId);
                                }*/
                                ...
                                //gridSystem.GetBlocksOfType(inBlocks);
                                //List<IMySlimBlock> slimList = new List<IMySlimBlock>();
                                //cubeGrid.GetBlocks(slimList, x => x.FatBlock is ModIngame.IMyTerminalBlock);
                                //MyVisualScriptLogicProvider.ShowNotificationToAll($"Terminal Count = {inBlocks.Count}", 5000, "Red");
                                smallProcesses.Enqueue(entity);
                                /*if (inBlocks.Count >= 100)
                                {
                                    largeProcesses.Add(entity);
                                }
                                else
                                {
                                    smallProcesses.Add(entity);
                                }*/
                            ...
                            {
                                Logging.Instance.WriteLine(String.Format("FindOnlineError: {0}", ex.ToString()));
                            }
                        }

                        // Rebuild our sort list
                        //if (ShouldRebuild)
                        RebuildSortListFromEntities(onlineEntities);
                    ...
                    //MyVisualScriptLogicProvider.ShowNotificationToAll($"small entities = {smallProcesses.Count}", 5000, "Red");
                    //MyVisualScriptLogicProvider.ShowNotificationToAll($"large entities = {largeProcesses.Count}", 5000, "Red");
                    //MyVisualScriptLogicProvider.ShowNotificationToAll($"online entities = {onlineEntities.Count}", 5000, "Red");

                    /*if (smallProcesses.Count != 0)
                    {
                        FindInventories();
                        //m_queueReady = true;
                        //return;
                    ...
                    /*if (index >= largeProcesses.Count - 1)
                    {
                        m_getOnlineEntities = true;
                        onlineEntities.Clear();
                        smallProcesses.Clear();
                        largeProcesses.Clear();
                        index = -1;
                        //MyVisualScriptLogicProvider.ShowNotificationToAll($"index reset", 3000, "Green");
                    }

                    if (largeProcesses.Count != 0)
                    {
                        FindInventories(largeProcesses, true);
                    }
                ...
                {
                    //m_queueReady = true;
                    //MyVisualScriptLogicProvider.ShowNotificationToAll($"Queue Ready = {m_queueReady}", 5000, "Red");
        ...
        public static void FindInventories()
        {
            MyAPIGateway.Parallel.StartBackground(() =>
            {
                try
                {
                    if (!Settings.Instance.GetInventories)
                    {
                        smallProcesses = new ConcurrentQueue<IMyEntity>();
                        return;
                    }

                    if (smallProcesses.Count == 0) return;
                    ...
                    List<IMyTerminalBlock> pulleeBlocks = new List<IMyTerminalBlock>();
                    List<IMyTerminalBlock> blocks = new List<IMyTerminalBlock>();
                    //List<IMyCubeBlock> cubeBlocks = new List<IMyCubeBlock>();

                    IMyEntity entity = null;
                    smallProcesses.TryDequeue(out entity);
                    if (entity == null || entity.MarkedForClose)
                        return;

                    //DateTime startGridLoop = DateTime.Now;

                    MyCubeGrid cubeGrid = (MyCubeGrid)entity;
                    pulleeBlocks.Clear();
                    pullerBlocks.Clear();
                    m_emptySet.Clear();

                    // Get a list to all the empty blocks on the grid.  We won't pull from those
                    //cubeGrid.GetBlocks(slimBlocks, x => x.FatBlock != null);
                    //var cubeBlocks = cubeGrid.GetFatBlocks();
                    Sandbox.ModAPI.Ingame.IMyGridTerminalSystem gridSystem = MyAPIGateway.TerminalActionsHelper.GetTerminalSystemForGrid(cubeGrid);
                    if (gridSystem == null)
                        return;
                    ...
                    //blocks = cubeBlocks.ConvertAll(x => (IMyTerminalBlock)x);
                    //MyVisualScriptLogicProvider.ShowNotificationToAll($"Block Count = {blocks.Count}", 3000, "Red");
                    /*foreach(var item in cubeBlocks)
                    {
                        var t = item as IMyTerminalBlock;
                        if (t == null) continue;
                        blocks.Add(t);
                    }*/
                    ...
                    if (pulleeBlocks.Count == 0) return;
                    var data = new Inventories();
                    data.allBlocks = new List<IMyTerminalBlock>(blocks);
                    data.pulleeBlocks = new List<IMyTerminalBlock>(pulleeBlocks);
                    inventories.Enqueue(data);
                    queueInventories = true;
                }
                catch (Exception ex)
                {
                    Logging.Instance.WriteLine(String.Format("SortInventoryLoopError(): {0}", ex.ToString()));
                }
            });
        }

        public static void CompareInventories()
        {
            MyAPIGateway.Parallel.StartBackground(() =>
            {
                try
                {
                    ...
                    Inventories data;
                    if (!inventories.TryDequeue(out data)) return;

                    List<IMyTerminalBlock> blocks = data.allBlocks;
                    List<IMyTerminalBlock> pulleeBlocks = data.pulleeBlocks;
        ...
        public static void ProcessQueue()
        {
            MyAPIGateway.Parallel.StartBackground(() =>
            {
                ...
                try
                {
                    //MyVisualScriptLogicProvider.ShowNotificationToAll($"Queue Pending = {m_queueItems.Count}", 500, "Red");
                    //m_queueItems.Clear();
                    ...
                    //MyVisualScriptLogicProvider.ShowNotificationToAll($"Queue Pending = {m_queueItems.Count}", 1000, "White");
            ...
            try
            {
                ...
                foreach (IMyEntity grid in entities)
                {
                    ...
                    foreach (IMySlimBlock slimblock in slimBlocks)
                    {
                        ...
                        m_cargoDictionary.TryAdd(entity.EntityId, components);
        ...
        /// </summary>
        /// <param name="entities">A list of all the entities we want to check</param>
        private static void RebuildSortListFromEntities(List<IMyEntity> entities)
        {
            m_rebuild = false;
            m_lastRebuild = DateTime.Now;
            DateTime start = DateTime.Now;
            ...
            try
            {
                ...
                foreach (IMyEntity grid in entities)
                {
                    ...
                    foreach (IMyTerminalBlock block in blocks)
                    {
                        ...
                        m_cargoDictionary.TryAdd(block.EntityId, components);
        ...
        private static void AddToSplitGroup(SortDefinitionItem item)
        {
            ...
            else
            {
                ...
                m_splitGroups.TryAdd(item.Definition, list);
            }
        }
        ...
        {
            if (!Settings.Instance.AddToQueue) return;
            MyDefinitionBase def = item.Definition;
